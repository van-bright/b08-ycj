...
async function defaultGasLimit(cntr: string, hexData: string) {
...
async function txnonce() {
  ...
  return Web3.utils.toHex(await web3.eth.getTransactionCount(pubkey));
}
...
async function send(): Promise<any> {
  // return new Promise(async (resolve, reject) => {
    ...
    const gasLimit = Web3.utils.toHex(await defaultGasLimit(contract, data));
